...
class WorkerAdminChat extends StatefulWidget {
  const WorkerAdminChat({Key? key,});



  @override
  _WorkerAdminChatState createState() => _WorkerAdminChatState();
}

class _WorkerAdminChatState extends State<WorkerAdminChat> {
  String _adminName = '';
  ...
  void initState() {
    ...
    _workerMessagesCollection = _firestore.collection('workers').doc( FirebaseAuth.instance.currentUser!.uid).collection('AWmessages');
    //_fetchAdminName();
  }
  ...
  Future<void> _handleImageSelectionAndUpload() async {
    ...
    if (pickedImage != null) {
      ...
      try {
        ...
        await _workerMessagesCollection.add({
          'AWmessage': imageUrl,
          'sender': 'Worker',
  ...
  Widget build(BuildContext context) {
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final message = messageData['AWmessage'] ?? '';
   ...
   Stream<List<DocumentSnapshot>> _mergeMessageStreams() {
    ...
    return  Rx.Rx.merge([workerStream, adminStream]);
  }
}
